...
def inicializaBusquedaHazBacktracking(anchHaz, tamMem, tipoProblema, estadoInicial):
    ...
    bASeleccionarDict[
        1] = 0  # es decir para el nivel 1 (el que surge de explorar el estado inicial) tenemos que se va explorar incialmente el primer bloque b
    ...
    def busquedaHazBacktracking(problema, coste):
        ...
        if (len(memoria) == tamMemoria):
            ...
            # len(memoria) es porque en memoria se guardan los niveles desde el 0 -> estado inicial
            nivelSiguienteAExplorar = len(memoria)
            bASeleccionarDict[nivelSiguienteAExplorar] = bASeleccionarDict[nivelSiguienteAExplorar] + 1

            print("Hacemos backtraking por memoria llena, volvemos al nivel: " + str(nivelSiguienteAExplorar) + "\n" + str(bAExplorar))
            return busquedaHazBacktracking(problema, coste - 1)
            ...
            if (len(estadosSucesoresEnBloques) <= bASeleccionarDict[len(memoria)]):
                ...
                print("Hacemos backtraking debido a que se han explorado todos los sucesores del nivel: " + str(nivelSiguienteAExplorar) + "\n" + str(bAExplorar))
                return busquedaHazBacktracking(problema, coste - 1)
    ...
    return busquedaHazBacktracking(problema, costeInicial)
