...
anchoDelHaz=10
tamDeMemoria=300 #siempre que la memoria sea <= al coste que nos retorna el algoritmo petara por llenar la memoria
...
for x in range(0, 100): #Generamos los 100 problemas y los ejecutamos
    estadoInicial = random.sample(range(0,16), 16) #Devuelve una lista única de 9 elementos elegidos de entre la secuencia de población.

    print("    ")
    print("||------ Ejecución número " + str(x+1) + "------||")
    print("||15-Puzzle, estado inicial: " + str(estadoInicial) + "||")
    print(inicializaBusquedaHaz(anchoDelHaz, tamDeMemoria, tipoProblema, estadoInicial))
    print("||-------------------------------------------------||")
